...
<div class="max-w-2xl">
    ...
    <div class="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
        <form action="{{ route('gedung.update', $gedung->id) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="alamat" rows="3"
                          class="w-full px-3 py-2.5 rounded-lg bg-zinc-800 border border-zinc-700 text-white text-sm placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500">{{ old('alamat', $gedung->alamat) }}</textarea>
                @error('alamat')
                    <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="no_telepon" value="{{ old('no_telepon', $gedung->no_telepon) }}"
                       class="w-full px-3 py-2.5 rounded-lg bg-zinc-800 border border-zinc-700 text-white text-sm placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500">
                @error('no_telepon')
                    <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="catatan" rows="2"
                          class="w-full px-3 py-2.5 rounded-lg bg-zinc-800 border border-zinc-700 text-white text-sm placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500">{{ old('catatan', $gedung->catatan) }}</textarea>
                @error('catatan')
                    <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status"
                    ...
                    <option value="aktif" {{ old('status', $gedung->status) === 'aktif' ? 'selected' : '' }}>Aktif</option>
                    <option value="nonaktif" {{ old('status', $gedung->status) === 'nonaktif' ? 'selected' : '' }}>Nonaktif</option>
                </select>
                @error('status')
                    <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                @enderror
